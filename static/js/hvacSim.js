...
function toggle(obj, min) {
    if(obj.isopened === false) {
        obj.isopened = true;
        writy(`Minute ${min}: ${obj.object} is opened`);
    } else {
        obj.isopened = false;
        writy(`Minute ${min}: ${obj.object} is closed`);
    }
}
...
function doorTime(darr, hv) { //Check if any door object's timer has passed a 5-minute mark; if not, pass; if so, change internal temp so that if external temp has a 10+ deg F difference, +/- 2 deg F to internal temp  
  for(let k = 0; k < 3; k++) {
    if (Number(darr[k].minutesopen) % 5 === 0 && Number(darr[k].minutesopen) != 0) {
      writy(`\t\t - ${darr[k].object} has been opened for 5 minutes.`);
      if (hv.outsideTemp >= (hv.insideTemp + 10)) {
        hv.insideTemp += 2;
        writy(`\t\t  HVAC - internal temp has increased by 2°F.`);
      } else if (hv.outsideTemp <= (hv.insideTemp - 10)) {
        hv.insideTemp -= 2;
        writy(`\t\t  HVAC - internal temp has decreased by 2°F.`);
      }
      writy(`\t\t\t External temperature: ${hv.outsideTemp}°F`);
      writy(`\t\t\t Internal temperature: ${hv.insideTemp}°F`);
    }  
  }
}
...
function windTime(warr, ac) { //Check if any window object's timer has passed a 5-minute mark; if not, pass; if so, change internal temp so that if external temp has a 10+ deg F difference, +/- 1 deg F to internal temp  
  for(let k = 0; k < 13; k++) {
    if (Number(warr[k].minutesopen) % 5 === 0 && Number(warr[k].minutesopen) != 0) {
      writy(`\t\t - ${warr[k].object} has been opened for 5 minutes.`);
      if (ac.outsideTemp >= (ac.insideTemp + 10)) {
        ac.insideTemp += 2;
        writy(`\t\t  HVAC - internal temp has increased by 1°F.`);
      } else if (ac.outsideTemp <= (ac.insideTemp - 10)) {
        ac.insideTemp -= 2;
        writy(`\t\t  HVAC - internal temp has decreased by 1°F.`);
      }
      writy(`\t\t\t External temperature: ${ac.outsideTemp}°F`);
      writy(`\t\t\t Internal temperature: ${ac.insideTemp}°F`);
    }  
  }
}
...
function closeTime(darr, warr, vac) { //Calculate the temperature change when all doors and windows are closed; +/- 2 deg F every hour for every 10+ deg F external temp difference
  ...
  writy(`\t Total temperature change for all doors and windows closed: ${n}`);
  writy(`\t\t External temperature: ${vac.outsideTemp}°F`);
  writy(`\t\t Internal temperature: ${vac.insideTemp}°F`);
}
...
writy(`Starting external temperature: ${hvac.outsideTemp}°F`);
writy(`Starting internal temperature: ${hvac.insideTemp}°F`);
...
for(let i = 1; i <= t; i++) {
    ...
    for(let j = 1; j <= e; j++) {
        ...
        writy(`\t - ${doors[n].object} opened for 30 seconds.`);
        doorTime(doors, hvac);

        n = rangedom(0, 12); //Rolls between 13 windows to open for 1 minute in opening event
        windows[n].minutesopen += 1;
        writy(`\t - ${windows[n].object} opened for 1 minute.`);
        windTime(windows, hvac);
        
        var y = rangedom(-2, 2);
        hvac.outsideTemp += Number(y);
        if (y != 0) {
          writy(`\t\t HAVC - External temp changed by ${y}°F.`);
        }
    }
    closeTime(doors, windows, hvac);
    resetime(doors, windows, hvac);
    hvac.insideTemp = rangedom(60, 80);
    writy(`\t HVAC - Internal temp changed to ${hvac.insideTemp}°F.`);
}
